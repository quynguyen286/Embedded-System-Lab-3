...
// ========== Handlers ==========
void handleRoot() { server1.send(200, "text/html", mainPage()); }

void handleToggle()
{
  int led = server1.arg("led").toInt();
  if (led == 1)
  {
  ...
  server1.send(200, "application/json",
...
void handleSensors()
{
  ...
  server1.send(200, "application/json", json);
}

void handleSettings() { server1.send(200, "text/html", settingsPage()); }

void handleConnect()
{
  wifi_ssid = server1.arg("ssid");
  wifi_password = server1.arg("pass");
  server1.send(200, "text/plain", "Connecting....");
  isAPMode = false;
  connecting = true;
...
void setupServer()
{
  ...
  server1.on("/toggle", HTTP_GET, handleToggle);
  server1.on("/sensors", HTTP_GET, handleSensors);
  server1.on("/settings", HTTP_GET, handleSettings);
  server1.on("/connect", HTTP_GET, handleConnect);
  server1.begin();
}
